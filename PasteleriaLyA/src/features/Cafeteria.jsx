...
export const VistaDetalleCuenta = ({ sesion, productos, onCerrar, onAgregarProducto, onPagarCuenta, onActualizarProducto, onCancelarCuenta, onDividirCuentaManual, onDesunirCuentas }) => {
    ...
    // --- NUEVO ESTADO: VISIBILIDAD DE CONTROLES DE PAGO ---
    ...
    return (
        <div className="fixed inset-0 bg-gray-100 z-[60] flex animate-fade-in-up">
            {/* LADO IZQUIERDO: MENÚ DE PRODUCTOS (Sin cambios) */}
                ...
                <div className="w-full md:w-96 bg-white shadow-2xl flex flex-col h-full border-l border-gray-200 animate-fade-in">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                        {(!sesion.cuenta || sesion.cuenta.length === 0) ? ( 
                            ...
                            <>
                                {/* GRUPO 1: PEDIDO ORIGINAL DEL CLIENTE */}
                                {sesion.cuenta.some(i => i.origen !== 'personal') && (
                                    <div className="mb-4">
                                        ...
                                        <div className="space-y-3">
                                            {sesion.cuenta.filter(i => i.origen !== 'personal').map((item, idx) => {
                                                ...
                                                return ( 
                                                    <div key={`cliente-${idx}`} className={`flex flex-col border-b border-gray-50 last:border-0 pb-2 gap-2 ${estaPausado ? 'opacity-80 bg-gray-50/50' : ''}`}>
                                                        <div className="flex justify-between items-start">
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2"><p className="font-bold text-gray-800 text-sm">{item.nombre}</p>{estaPausado && <span className="bg-red-100 text-red-600 text-[9px] px-1 rounded">PAUSADO</span>}</div>
                                                                <p className="text-xs text-gray-500">{item.categoria}</p>
                                                            </div>
                                                            <div className="text-right"><p className="font-bold text-gray-900">${subtotalItem.toFixed(2)}</p></div>
                                                        </div>
                                                        <div className="flex items-center justify-end gap-3 bg-gray-50 p-1 rounded-lg w-fit self-end">
                                                            <button onClick={() => onActualizarProducto(sesion.id, item.id, -1)} className="text-gray-400 hover:text-orange-600 p-1"><MinusCircle size={16}/></button>
                                                            <span className="font-bold text-gray-700 text-xs w-4 text-center">{cantidad}</span>
                                                            <button onClick={() => !estaPausado && onActualizarProducto(sesion.id, item.id, 1)} className="text-gray-400 hover:text-orange-600 p-1"><PlusCircle size={16}/></button>
                                                            <div className="w-px h-3 bg-gray-300 mx-1"></div>
                                                            <button onClick={() => onActualizarProducto(sesion.id, item.id, -cantidad)} className="text-red-300 hover:text-red-500 p-1"><Trash2 size={16}/></button>
                                                        </div>
                                                    </div> 
                                                );
                                            })}
                                ...
                                {/* GRUPO 2: AGREGADOS POR PERSONAL */}
                                {sesion.cuenta.some(i => i.origen === 'personal') && (
                                    <div className="mb-2 mt-4">
                                        ...
                                        <div className="space-y-3 bg-blue-50/30 p-2 rounded-b border border-t-0 border-blue-50">
                                            {sesion.cuenta.filter(i => i.origen === 'personal').map((item, idx) => {
                                                const cantidad = item.cantidad || 1; const subtotalItem = item.precio * cantidad; const productoReal = productos.find(p => p.id === item.id); const estaPausado = productoReal?.pausado;
                                                return ( 
                                                    <div key={`personal-${idx}`} className="flex flex-col border-b border-blue-100 last:border-0 pb-2 gap-2">
                                                        <div className="flex justify-between items-start">
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2"><p className="font-bold text-gray-800 text-sm">{item.nombre}</p></div>
                                                            </div>
                                                            <div className="text-right"><p className="font-bold text-blue-900">${subtotalItem.toFixed(2)}</p></div>
                                                        </div>
                                                        <div className="flex items-center justify-end gap-3 bg-white border border-blue-100 p-1 rounded-lg w-fit self-end">
                                                            <button onClick={() => onActualizarProducto(sesion.id, item.id, -1)} className="text-gray-400 hover:text-blue-600 p-1"><MinusCircle size={16}/></button>
                                                            <span className="font-bold text-gray-700 text-xs w-4 text-center">{cantidad}</span>
                                                            <button onClick={() => !estaPausado && onActualizarProducto(sesion.id, item.id, 1)} className="text-gray-400 hover:text-blue-600 p-1"><PlusCircle size={16}/></button>
                                                            <div className="w-px h-3 bg-gray-300 mx-1"></div>
                                                            <button onClick={() => onActualizarProducto(sesion.id, item.id, -cantidad)} className="text-red-300 hover:text-red-500 p-1"><Trash2 size={16}/></button>
                                                        </div>
                                                    </div> 
                    ...
                    <div className="p-6 bg-gray-50 border-t border-gray-200 transition-all duration-300">
                        {/* HEADER DEL FOOTER (SIEMPRE VISIBLE) */}
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button 
                                    ...
                                    className="p-1 bg-gray-200 rounded-full hover:bg-gray-300 text-gray-600 transition"
