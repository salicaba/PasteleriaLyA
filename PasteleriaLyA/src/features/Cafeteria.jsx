...
export const VistaDetalleCuenta = ({ sesion, productos, onCerrar, onAgregarProducto, onPagarCuenta, onActualizarProducto, onCancelarCuenta, onDividirCuentaManual, onDesunirCuentas }) => {
    ...
    // --- NUEVO ESTADO: VISIBILIDAD DE COMANDA (LÓGICA RESPONSIVE) ---
    // En pantallas grandes (>= 768px), inicia abierto (true).
    // En celulares (< 768px), inicia cerrado (false) para mostrar menú primero.
    ...
    return (
        <div className="fixed inset-0 bg-gray-100 z-[60] flex animate-fade-in-up">
            ...
            {/* Si comandaVisible es TRUE (y estamos en móvil), ocultamos el menú con 'hidden md:flex' */}
            {/* Si comandaVisible es FALSE, mostramos menú completo 'flex' */}
            <div className={`${comandaVisible ? 'hidden md:flex' : 'flex'} flex-1 flex-col h-full overflow-hidden border-r border-gray-300 relative`}>
                ...
                {/* --- BOTÓN FLOTANTE PARA VER CUENTA (SOLO SI ESTÁ OCULTA) --- */}
                {!comandaVisible && (
                    <div className="absolute bottom-6 right-6 z-50 animate-bounce-in">
                        <button 
                            onClick={() => setComandaVisible(true)}
                            className="bg-gray-900 text-white px-6 py-4 rounded-full shadow-2xl flex items-center gap-3 font-bold text-lg hover:bg-gray-800 transition transform hover:scale-105 border-2 border-orange-500"
                        >
                            <Receipt size={24} className="text-orange-400"/> 
                            <span>Ver Cuenta</span>
                            <span className="bg-white text-gray-900 px-2 py-0.5 rounded text-sm">${total.toFixed(2)}</span>
                        </button>
                    </div>
                )}
            </div>

            {/* LADO DERECHO: CUENTA / TICKET - VISIBLE SOLO SI COMANDA VISIBLE ES TRUE */}
            {comandaVisible && (
                // En móvil usamos w-full para que ocupe todo. En PC (md) usamos w-96 fijo.
                <div className="w-full md:w-96 bg-white shadow-2xl flex flex-col h-full border-l border-gray-200 animate-fade-in">
                    <div className="p-6 bg-gray-900 text-white">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex items-center gap-3">
                                {/* BOTÓN PARA OCULTAR */}
                                <button onClick={() => setComandaVisible(false)} className="text-gray-400 hover:text-white transition p-1 rounded-md hover:bg-gray-800" title="Ocultar Comanda">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        {(!sesion.cuenta || sesion.cuenta.length === 0) ? ( <div className="text-center text-gray-400 py-10 italic">Cuenta vacía.<br />Selecciona productos.</div> ) : ( sesion.cuenta.map((item, idx) => {
                                const cantidad = item.cantidad || 1; const subtotalItem = item.precio * cantidad; const productoReal = productos.find(p => p.id === item.id); const estaPausado = productoReal?.pausado;
                                return ( <div key={idx} className={`flex flex-col border-b border-gray-100 pb-3 last:border-0 gap-2 ${estaPausado ? 'opacity-80 bg-gray-50/50 -mx-2 px-2 py-1 rounded' : ''}`}>
                                        <div className="flex justify-between items-start"><div className="flex-1"><div className="flex items-center gap-2"><p className="font-bold text-gray-800 text-sm">{item.nombre}</p>{estaPausado && (<span className="bg-red-100 text-red-600 text-[9px] font-bold px-1.5 py-0.5 rounded border border-red-200 flex items-center gap-0.5"><Lock size={8} /> PAUSADO</span>)}</div><p className="text-xs text-gray-500">{item.categoria}</p></div><div className="text-right"><p className="font-bold text-gray-900">${subtotalItem.toFixed(2)}</p>{cantidad > 1 && <p className="text-[10px] text-gray-400">${item.precio} c/u</p>}</div></div>
                                        <div className="flex items-center justify-end gap-3 bg-gray-50 p-1.5 rounded-lg w-fit self-end shadow-sm"><button onClick={() => onActualizarProducto(sesion.id, item.id, -1)} className="text-gray-400 hover:text-orange-600 hover:bg-white p-1 rounded transition" title="Restar 1"><MinusCircle size={18}/></button><span className="font-bold text-gray-700 w-4 text-center text-sm">{cantidad}</span><button onClick={() => !estaPausado && onActualizarProducto(sesion.id, item.id, 1)} disabled={estaPausado} className={`p-1 rounded transition ${estaPausado ? 'text-gray-300 cursor-not-allowed' : 'text-gray-400 hover:text-orange-600 hover:bg-white'}`} title={estaPausado ? "Producto Pausado" : "Sumar 1"}><PlusCircle size={18}/></button><div className="w-px h-4 bg-gray-300 mx-1"></div><button onClick={() => onActualizarProducto(sesion.id, item.id, -cantidad)} className="text-red-400 hover:text-red-600 hover:bg-white p-1 rounded transition" title="Eliminar producto"><Trash2 size={18}/></button></div>
                                    </div> );
                            }) )}
                    </div>
                    <div className="p-6 bg-gray-50 border-t border-gray-200">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                                {/* BOTÓN 1: DIVIDIR POR ITEMS (MANUAL) - SOLO EN MESAS */}
                                {sesion.tipo === 'mesa' && (
                                    <button onClick={() => setModalDividirManualOpen(true)} className="flex-1 mt-2 py-2 rounded-lg text-xs font-bold text-purple-500 bg-purple-50 hover:bg-purple-100 hover:text-purple-700 flex justify-center items-center gap-1 transition"><Split size={14}/> Separar por Items</button>
                                )}
                                
                                {/* BOTÓN 2: DESUNIR CUENTAS (HISTÓRICO) - SOLO EN MESAS Y SI HAY HISTORIAL */}
                                {sesion.tipo === 'mesa' && sesion.historicoFusion && sesion.historicoFusion.length > 0 && (
                                    <button onClick={() => setModalDesunirOpen(true)} className="flex-1 mt-2 py-2 rounded-lg text-xs font-bold text-indigo-500 bg-indigo-50 hover:bg-indigo-100 hover:text-indigo-700 flex justify-center items-center gap-1 transition"><Undo2 size={14}/> Deshacer Unión</button>
                                )}
