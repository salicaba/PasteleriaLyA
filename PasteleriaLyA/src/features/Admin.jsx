        ...
        <div className="p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex flex-wrap gap-3 bg-white p-3 rounded-xl shadow-sm border border-gray-200 items-end">
                    <div>
                        ...
                        <input type="month" value={mesSeleccionado} min="2025-12" onChange={(e) => { setMesSeleccionado(e.target.value); limpiarRango(); }} className="border rounded-lg p-2 text-sm font-bold text-gray-700 bg-gray-50 hover:bg-white transition" />
                    </div>
                    <div className="h-10 w-px bg-gray-300 mx-2 hidden md:block"></div>
                    <div className="flex items-end gap-2">
                        <div><label className="text-xs font-bold text-gray-500 block mb-1">Desde</label><input type="date" value={rangoInicio} min="2025-12-01" onChange={(e) => setRangoInicio(e.target.value)} className="border rounded-lg p-2 text-sm text-gray-600 w-32" /></div>
                        <div><label className="text-xs font-bold text-gray-500 block mb-1">Hasta</label><input type="date" value={rangoFin} min="2025-12-01" onChange={(e) => setRangoFin(e.target.value)} className="border rounded-lg p-2 text-sm text-gray-600 w-32" /></div>
                    </div>
                    {(rangoInicio || rangoFin) && (<button onClick={limpiarRango} className="text-xs text-red-500 font-bold hover:underline mb-3 flex items-center gap-1"><X size={12} /> Limpiar Rango</button>)}
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-gray-700 flex items-center gap-2 capitalize"><BarChart3 size={20} /> {datosReporte.tituloPeriodo}</h3>
                </div>
