...
import { 
    ...
    Minus, Plus, Info, ChevronUp, ChevronDown // <-- Importantes para el modal
} from 'lucide-react';
...
// --- TARJETA DE PRODUCTO ---
// --- TARJETA DE PRODUCTO (ACTUALIZADA: BOTÓN AGREGAR VISIBLE) ---
export const CardProducto = ({ producto, onClick, onAdd }) => { // <--- Agregamos prop onAdd
    const esImagen = (str) => str && (str.startsWith('http') || str.startsWith('data:image'));
    const estaPausado = producto.pausado === true;

    return (
        <div 
            // El clic general abre los detalles (onClick)
        ...
        >
            ...
            <div className="relative w-full h-36 sm:h-40 bg-gray-50 flex items-center justify-center p-4 overflow-hidden">
                ...
                >
                    {esImagen(producto.imagen) ? (
                        ...
                        <span className="text-5xl select-none filter drop-shadow-sm opacity-80">
                            {producto.imagen || '☕'}
                        </span>
                    )}
                </div>
            ...
            <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between">
                <div>
                    <h4 className={`font-bold text-gray-800 text-sm leading-tight mb-1 line-clamp-2 ${estaPausado ? 'text-gray-400' : ''}`}>
                        {producto.nombre}
                    </h4>
                    <p className="text-xs text-gray-500 mb-2 line-clamp-2 leading-relaxed hidden sm:block">
                        {producto.descripcion || "Sin descripción."}
                    </p>
                </div>
                
                <div className="flex justify-between items-center border-t border-gray-50 pt-2 mt-1">
                    <span className={`text-lg font-bold ${estaPausado ? 'text-gray-400 line-through' : 'text-orange-600'}`}>
                        ${parseFloat(producto.precio).toFixed(2)}
                    </span>
                    
                    {/* BOTÓN DE AGREGAR RÁPIDO (Siempre visible si hay onAdd) */}
                    {!estaPausado && onAdd && (
                        <button
                            onClick={(e) => {
...
// --- MODAL INFO DE PRODUCTO (CORREGIDO PARA EVITAR ERROR REACT 19) ---
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-bounce-in flex flex-col max-h-[85vh]">
                ...
                <div className="relative h-48 bg-gray-100 flex items-center justify-center shrink-0">
                    ...
                    <button onClick={onClose} className="absolute top-3 right-3 bg-white/90 hover:bg-white p-2 rounded-full text-gray-500 hover:text-gray-800 transition shadow-sm border border-gray-200 z-10">
                        <X size={18}/>
                    </button>
                </div>
                ...
                {/* Footer (CORREGIDO: Estructura HTML válida) */}
                <div className="bg-white border-t border-gray-100 shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.03)] transition-all duration-300">
                    
                    {!mostrarControles ? (
                        // VISTA COLAPSADA (Ahora es un botón completo, no un div con botón dentro)
                        <button 
                            onClick={() => setMostrarControles(true)}
                            className="w-full p-4 hover:bg-gray-50 transition-colors flex flex-col items-center justify-center group outline-none"
                        >
                            <div className="w-8 h-1 bg-gray-200 rounded-full mb-2 group-hover:bg-orange-200 transition-colors"></div>
                            <span className="text-sm font-bold text-orange-600 flex items-center gap-2 animate-pulse">
                                Quiero pedir esto <ChevronUp size={16}/>
                            </span>
                        </button>
                    ) : (
                        // VISTA EXPANDIDA
                        <div className="p-5 animate-fade-in-up">
                            <div className="flex justify-center mb-2">
                                <button onClick={() => setMostrarControles(false)} className="text-gray-400 hover:text-gray-600 p-1"><ChevronDown size={20}/></button>
                            </div>
                            ...
                            <button 
                                onClick={() => { onAgregar(producto, cantidad); onClose(); }} 
                                className="w-full py-3.5 bg-gray-900 text-white rounded-xl font-bold shadow-lg hover:bg-gray-800 transition transform active:scale-95 flex items-center justify-center gap-2 text-sm md:text-base"
                            >
                                <ShoppingBag size={18}/> Agregar - ${(producto.precio * cantidad).toFixed(2)}
                            </button>
                        </div>
                    )}
                </div>
...
// ... (Resto de componentes: BotonNav, Sidebar, LayoutConSidebar, Modales de Confirmación, etc. SE MANTIENEN IGUAL)
...
export const ModalConfirmacion = ({ isOpen, onClose, onConfirm, titulo = "¿Estás seguro?", mensaje = "Esta acción no se puede deshacer.", tipo = "eliminar" }) => { if (!isOpen) return null; const esPago = tipo === 'pago'; const estilos = esPago ? { borde: "border-green-500", icono: <div className="bg-green-100 p-3 rounded-full w-fit mx-auto mb-4 animate-bounce-in"><DollarSign size={32} className="text-green-600" /></div>, boton: "bg-green-600 hover:bg-green-700 text-white" } : { borde: "border-red-500", icono: <Trash2 size={40} className="text-red-500 mx-auto mb-4" />, boton: "bg-red-600 hover:bg-red-700 text-white" }; return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[280] p-4 backdrop-blur-sm transition-all"> <div className={`bg-white rounded-2xl shadow-2xl w-full max-w-xs sm:max-w-sm md:max-w-md overflow-hidden animate-bounce-in transform border-t-8 ${estilos.borde}`}> <div className="p-6 text-center"> {estilos.icono} <h3 className="text-xl font-bold text-gray-900 mb-2">{titulo}</h3> <p className="text-gray-500 mb-6 text-sm">{mensaje}</p> <div className="flex space-x-3 justify-center"><button onClick={onClose} className="px-5 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition">Cancelar</button><button onClick={onConfirm} className={`px-5 py-2.5 rounded-lg font-bold shadow-lg hover:shadow-xl transition transform active:scale-95 ${estilos.boton}`}>Confirmar</button></div> </div> </div> </div> ); };
export const ModalDetalles = ({ pedido, cerrar, onRegistrarPago }) => { if (!pedido) return null; const numPagos = pedido.numPagos ? parseInt(pedido.numPagos) : 1; const esPagoUnico = numPagos === 1; const [montoRecibido, setMontoRecibido] = useState(''); const [esLiquidacion, setEsLiquidacion] = useState(esPagoUnico); const montoTotal = parseFloat(pedido.total); const pagosRealizados = pedido.pagosRealizados || (pedido.origen === 'Cafetería' ? 1 : 0); const montoPorPago = (montoTotal / numPagos); const saldoPendiente = (montoTotal - (montoPorPago * pagosRealizados)); const montoACobrar = esLiquidacion ? saldoPendiente : montoPorPago; const cambio = montoRecibido ? (parseFloat(montoRecibido) - montoACobrar) : 0; const esMontoSuficiente = montoRecibido && parseFloat(montoRecibido) >= montoACobrar - 0.01; const porcentajePagado = (pagosRealizados / numPagos) * 100; const esCafeteria = pedido.origen === 'Cafetería'; const pagosRestantes = numPagos - pagosRealizados; const handleCobrar = () => { if (esMontoSuficiente) { onRegistrarPago(pedido.folio, esLiquidacion); setMontoRecibido(''); setEsLiquidacion(esPagoUnico); } }; return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[270] p-4 backdrop-blur-sm"> <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md md:max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col"> <div className="bg-pink-900 p-4 md:p-6 text-white flex justify-between shrink-0"><div><h3 className="text-xl md:text-2xl font-bold">{pedido.cliente}</h3><p className="text-pink-200 font-mono text-sm">{pedido.folioLocal || pedido.folio || pedido.id}</p></div><button onClick={cerrar}><X /></button></div> <div className="p-4 md:p-6 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar"> <div className="bg-gray-50 p-4 rounded-xl border"><h4 className="text-sm font-bold text-gray-500 uppercase flex items-center mb-2"><ShoppingBag size={14} className="mr-2" /> Descripción</h4><span className={`text-xs font-bold px-2 py-1 rounded-full uppercase mb-2 inline-block ${esCafeteria ? 'bg-orange-100 text-orange-700' : 'bg-pink-100 text-pink-700'}`}>{pedido.tipoProducto || (esCafeteria ? 'Consumo Cafetería' : 'Producto')}</span><p className="text-gray-800 whitespace-pre-wrap text-sm md:text-base">{pedido.detalles || (esCafeteria ? `Venta de mostrador con ${pedido.items} artículos.` : 'Sin detalles.')}</p></div> <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">{pedido.telefono && <div><p className="text-xs text-gray-400">Teléfono</p><p className="font-medium text-gray-700">{pedido.telefono}</p></div>}{pedido.fechaEntrega && <div><p className="text-xs text-gray-400">Entrega Programada</p><p className="font-medium text-pink-700 flex items-center gap-1"><CalendarDays size={14} />{formatearFechaLocal(pedido.fechaEntrega)}</p></div>}<div><p className="text-xs text-gray-400">Fecha Registro</p><p className="font-medium text-gray-700 flex items-center gap-1"><CalendarDays size={14} />{formatearFechaLocal(pedido.fecha)}</p></div></div> {!esCafeteria && (<div className="flex flex-col sm:flex-row gap-2"><button onClick={() => imprimirTicket(pedido, 'comanda')} className="flex-1 py-2 bg-pink-50 hover:bg-pink-100 rounded-lg text-xs font-bold flex items-center justify-center gap-2 text-pink-700"><FileText size={16}/> Comanda Cocina</button></div>)} <hr className="border-gray-100" /> {pedido.estado === 'Cancelado' ? (<div className="w-full bg-red-100 text-red-800 font-bold py-6 rounded-xl text-center flex flex-col justify-center items-center border border-red-200"><AlertCircle size={32} className="mb-2" /><span className="text-lg">Pedido cancelado</span></div>) : (<div><div className="flex justify-between items-end mb-2"><span className="text-3xl font-bold text-gray-800">${montoTotal.toFixed(2)}</span><span className="text-sm text-gray-500 mb-1">Total</span></div><div className="w-full bg-gray-200 rounded-full h-4 mb-4 overflow-hidden"><div className="bg-green-500 h-4 rounded-full transition-all duration-500" style={{ width: `${porcentajePagado}%` }}></div></div>{!esCafeteria && <div className="flex justify-between text-sm text-gray-600 mb-6"><span>Pagado: {pagosRealizados} de {numPagos}</span><span>Resta: ${saldoPendiente.toFixed(2)}</span></div>}{pagosRestantes > 0 ? (<div className="bg-pink-50 rounded-xl p-4 md:p-5 border border-pink-100"><h5 className="font-bold text-pink-800 mb-3 flex items-center"><Calculator size={16} className="mr-2" /> Cajero</h5>{!esPagoUnico ? (<div className="flex space-x-2 mb-4"><button onClick={() => { setEsLiquidacion(false); setMontoRecibido(''); }} className={`flex-1 py-2 rounded-lg text-sm font-bold transition ${!esLiquidacion ? 'bg-pink-600 text-white' : 'bg-white border'}`}>Abono (${montoPorPago.toFixed(2)})</button><button onClick={() => { setEsLiquidacion(true); setMontoRecibido(''); }} className={`flex-1 py-2 rounded-lg text-sm font-bold transition ${esLiquidacion ? 'bg-pink-600 text-white' : 'bg-white border'}`}>Liquidar (${saldoPendiente.toFixed(2)})</button></div>) : (<div className="mb-4 p-2 bg-pink-100 text-pink-800 text-center rounded-lg text-sm font-bold border border-pink-200">Pago de Contado (Total: ${saldoPendiente.toFixed(2)})</div>)}<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4"><div><label className="text-xs font-bold text-gray-500">Recibido</label><input type="number" min="0" className="w-full p-2 rounded border font-bold" value={montoRecibido} onChange={e => setMontoRecibido(e.target.value)} /></div><div><label className="text-xs font-bold text-gray-500">Cambio</label><div className="w-full p-2 rounded border font-bold bg-white text-green-600">${cambio >= 0 ? cambio.toFixed(2) : '0.00'}</div></div></div><button onClick={handleCobrar} disabled={!esMontoSuficiente} className={`w-full py-3 rounded-xl font-bold shadow-lg text-white ${esMontoSuficiente ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-300'}`}>Confirmar Cobro</button></div>) : (<div className="w-full bg-green-100 text-green-800 font-bold py-3 rounded-xl text-center border border-green-200">¡Pagado / Liquidado!</div>)}</div>)} </div> </div> </div> ); };
