...
const BotonNav = ({ icon, label, active, onClick, colorTheme = "pink" }) => {
    ...
    return (
        <button 
            onClick={onClick} 
            className={`w-full flex items-center space-x-3 px-3 md:px-4 py-2 md:py-3 rounded-lg transition-colors text-sm md:text-base ${active ? activeClass : hoverClass}`}
        >
            {icon} 
        ...
        <>
            ...
            `}>
                ...
                {isOpen && (
                    <button 
                        onClick={toggleSidebar}
                        className="absolute top-4 right-4 p-1 hover:bg-white/20 rounded-full md:hidden"
                    >
            ...
            <div className="flex-1 flex flex-col w-full h-full relative transition-all duration-300">
                ...
                <header className="md:hidden bg-white shadow-sm border-b px-4 py-3 flex items-center justify-between z-40 shrink-0">
                    ...
                    <h1 className="text-xl font-bold" style={{ fontFamily: "'Dancing Script', cursive" }}>
                        {modo === 'admin' ? 'Administración' : 
                         modo === 'pasteleria' ? 'Pastelería' : 'Cafetería'}
                    </h1>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md md:max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
                ...
                <div className="p-4 md:p-6 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar">
                    <div className="bg-gray-50 p-4 rounded-xl border">
                        <h4 className="text-sm font-bold text-gray-500 uppercase flex items-center mb-2"><ShoppingBag size={14} className="mr-2" /> Descripción</h4>
                        ...
                        <p className="text-gray-800 whitespace-pre-wrap text-sm md:text-base">{pedido.detalles || (esCafeteria ? `Venta de mostrador con ${pedido.items} artículos.` : 'Sin detalles.')}</p>
                    </div>
                    ...
                    {pedido.estado === 'Cancelado' ? (
                        <div className="w-full bg-red-100 text-red-800 font-bold py-6 rounded-xl text-center flex flex-col justify-center items-center border border-red-200">
                            <AlertCircle size={32} className="mb-2" /><span className="text-lg">Pedido cancelado</span>
                        </div>
                    ) : (
                        <div>
...
export const ModalVentasDia = ({ dia, mes, anio, ventas, cerrar, onVerDetalle }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[80] p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md md:max-w-lg max-h-[80vh] overflow-hidden flex flex-col animate-fade-in-up">
                <div className="bg-pink-900 p-4 flex justify-between items-center text-white sticky top-0 z-10 shrink-0">
                    <h3 className="font-bold text-lg flex items-center gap-2"><CalendarIcon size={20} /> Ventas del {dia}/{mes + 1}/{anio}</h3>
                    <button onClick={cerrar}><X /></button>
                </div>
